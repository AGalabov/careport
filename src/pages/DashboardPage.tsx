...
function LastFillCard({
  ...
  const typeLabel =
    record.fuelType === 'lpg' ? t('fuel.toggleLpg') : t('fuel.togglePetrol');
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (reminders.length > 0) {
      void checkDateReminders(reminders, updateReminder, notifCopy);
    }
  }, [reminders, updateReminder, notifCopy]);
  ...
  if (cars.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] px-4 text-center">
        <p className="text-gray-500 mb-4">{t('dashboard.addCarPrompt')}</p>
        <button
