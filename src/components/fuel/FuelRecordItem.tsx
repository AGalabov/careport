...
export default function FuelRecordItem({
  ...
  const equivalentLabel =
    record.fuelType === 'lpg' ? t('fuel.item.petrolEquivLower') : t('fuel.item.lpgEquivLower');
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
      ...
      {record.notes && <p className="mt-2 text-xs text-gray-400 italic">{record.notes}</p>}
    </div>
  );
}
